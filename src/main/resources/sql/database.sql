...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `ServicesCar`.customer
(
    `id`         INT          NOT NULL AUTO_INCREMENT COMMENT 'The primary key of the table.',
...
(
    ...
    customer_id            INT          NOT NULL COMMENT 'Foreign key for table clients by id',
    `vehicle_id`           INT          NOT NULL COMMENT 'Foreign key for table vehicles by id',
    PRIMARY KEY (`id`),
    INDEX `fk_workers_has_clients_clients1_idx` (customer_id ASC) VISIBLE,
    ...
    CONSTRAINT `fk_workers_has_clients_clients1`
        FOREIGN KEY (customer_id)
            REFERENCES `ServicesCar`.`customer` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
...
-- -----------------------------------------------------
-- Table `ServicesCar`.`status`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `ServicesCar`.`status`
(
    `id`       INT         NOT NULL AUTO_INCREMENT COMMENT 'The primary key of the table.',
    `status`   TINYINT(10) NULL COMMENT 'If 1 then car is restored',
    `order_id` INT         NOT NULL COMMENT 'Foreign key for table order by id',
    PRIMARY KEY (`id`),
    INDEX `fk_workers_has_clients_order1_idx` (order_id ASC) VISIBLE,
    CONSTRAINT `fk_workers_has_clients_order1_idx`
        FOREIGN KEY (order_id)
            REFERENCES `ServicesCar`.`orders` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB
    COMMENT = 'Table containing status';